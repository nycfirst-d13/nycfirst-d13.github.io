...
2. `subpathsToRegions(subs)` — build a `parent` map by **bbox containment** (via paper.js `.area`/`.bounds`: smallest-area region whose bbox contains each contour), then **walk every contour to its top-level ancestor and union each connected nest into ONE even-odd compound `d`**. Correctness comes from even-odd itself — a point fills iff it's inside an **odd number of contours**, so holes, islands-in-holes, and holes-in-islands all resolve by crossing count at any depth. No depth-parity or direct-child bookkeeping (that older approach flipped a hole to a filled region on any bbox misnest → black-on-black).
3. Result: a flat `group` whose children are one `path` per connected nest, each `attrs: { d, fillRule: 'evenodd' }`, `processType: 'etch'`. Single-region traces collapse to a bare top-level path (no group).
...
**Ceiling:** containment is bbox-only, not true point-in-polygon (`ponytail:` comment in `subpathsToRegions`). This is now **safe for fill correctness** — bbox containment only ever over-merges (a real container's bbox always contains its child; a bbox can only falsely claim to contain), and over-merging disjoint contours into one even-odd compound still renders each correctly. The only cost is coarser selection grouping (e.g. a whole boot lands in one selectable path). Upgrade to `path.contains()` only if you need tighter per-shape selection, not for correct holes.
